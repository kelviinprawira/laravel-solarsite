...
class SolarChart extends Component
{
    ...
    public function updateChartData()
    {
        ...
        if ($chart) {
            ...
            $this->chart_wm_ach = [
                Number::format(($chart->wm_alat_berat_real / $chart->wm_alat_berat_plan) +
                    ($chart->wm_dt_infras_real /
                        $chart->wm_dt_infras_plan) + ($chart->wm_pompa_genset_real /
                        $chart->wm_pompa_genset_plan) +
                    ($chart->wm_unit_umum_real / $chart->wm_unit_umum_plan), 2)
            ];
            ...
            $this->chart_tp_ach = [
                Number::format(
                    ($chart->tp_dt_transport_real / $chart->tp_dt_transport_plan) +
                    ($chart->tp_dt_infras_real /
                        $chart->tp_dt_infras_plan), 2),
            ];
            ...
            $this->chart_rt_ach = [
                Number::format(($chart->rt_alat_rawat_real / $chart->rt_alat_rawat_plan) +
                    ($chart->rt_alat_tankos_real / $chart->rt_alat_tankos_plan), 2)
            ];
            ...
            $this->chart_ms_ach = [
                Number::format(($chart->ms_genset_real / $chart->ms_genset_plan) +
                    ($chart->ms_alat_pabrik_real /
                        $chart->ms_alat_pabrik_plan), 2),
            ];
            ...
            $this->chart_inf_ach = [
                Number::format(($chart->inf_alat_berat_real / $chart->inf_alat_berat_plan) +
                    ($chart->inf_dt_infras_real /
                        $chart->inf_dt_infras_plan) + ($chart->inf_pompa_genset_real / $chart->inf_pompa_genset_plan) +
                    ($chart->inf_unit_umum_real / $chart->inf_unit_umum_plan), 2),
            ];
        }
    }
